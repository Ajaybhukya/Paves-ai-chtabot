    ...
    if st.button("🎤 Speak", key="speak"):
        ...
        while keep_listening:
            ...
            if not user_input:
                ...
                time.sleep(3)
                st.rerun()
            elif ( "send email" in user_input.lower()) or (" sent email" in user_input.lower()) or ("sent mail" in user_input.lower()) or ("send mail" in user_input.lower()) or ("compose email" in user_input.lower()) or ("compose mail" in user_input.lower()):
        ...
        if st.button("📩 Send", key="send_button"):
            if user_query.strip() != "":
                if "take picture" in user_query.lower():
                    ...
                    st.session_state.show_text_input = False  # Hide text input after sending
                    st.rerun()
                elif ("send email" in user_query.lower()) or ("sent email" in user_query.lower()) or ("sent mail" in user_query.lower()) or ("send mail" in user_query.lower()) or ("compose email" in user_query.lower()):
                    st.session_state.compose_email = True
                    st.session_state.show_text_input = False  # Hide text input after sending
                    st.rerun()
...
if st.session_state.compose_email:
    ...
    if st.button("Send Email"):
        if is_valid_email(to_mail) and subject and message:
